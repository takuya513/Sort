...
public class QuickMergeSort4<E extends Comparable> implements Sort<E> {
	...
	public  void merge(int left,int mid,int right,Object[] buf){
		...
		//tt.p("OK");
		while(i <= mid) { buff[k] = array[i]; i++; k++;}
		while(j <= right) { buff[k] = array[j]; j++;k++;}
		for(i = left;i <= right; i++){ array[i] = (E) buff[i];}
		System.out.println("OKKKK2222");
	}

	class MyList<E extends Comparable>{
		E data;
		MyList nextList;
		public MyList(){
